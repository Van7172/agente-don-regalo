...
prompt del orquestador (`prompts/playbooks.ORCHESTRATOR`) no lleva estilo de
respuesta: contiene la taxonomía, el mapa de capacidades y la prioridad
comercial. Su pericia es elegir la etapa y la fuente correcta, no redactar ni
inventar productos, cobertura o precios.
...
| `catalog` | búsqueda de productos, campañas | 8 | **no** |
| `detail` | detalle de un producto ya mostrado | 2 | no |
| `coverage` | distrito y tarifa (determinista) | 1 | no |
...
Cobertura solo tiene permitida `distritos_cobertura`. Aunque la llamada sea
directa desde código, `registry.assert_tool_allowed` aplica el mismo mínimo
privilegio que en los especialistas LLM y la ejecución se refleja en `Trace`.
...
Una cuarta regla comercial tiene precedencia: un destino explícito («quiero
hacer un pedido para Cercado de Lima») va a `coverage`, confirma cobertura y
tarifa y luego pregunta por el regalo. Un distrito usado como referencia no
logística no secuestra el turno de catálogo. Las citas de WhatsApp se separan
en `Turn.quoted`: aportan contexto de producto, pero no enrutan.
